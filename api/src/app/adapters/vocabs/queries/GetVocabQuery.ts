...
import { GetVocabInputPort } from "../../../../core/vocabs/queries/inputs/GetVocabInputPort";
import { VocabReadModel } from "../../../../core/vocabs/queries/readModels/VocabReadModel";

import fs from "fs";
import path from "path";

export class GetVocabQuery implements IGetVocabQuery {
  async execute(input: GetVocabInputPort): Promise<VocabReadModel> {
    ...
    return lines
      ...
      .filter((line) => {
        ...
        return input.vocabId() === parseInt(id);
      })
      .map((line) => {
        let commaPositions = [];
        let searchStartIndex = 0;
